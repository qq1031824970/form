/*!
 * @form-create/arco-design v3.1.32
 * (c) 2018-2024 xaboy
...
(function (global, factory) {
    ...
    var Checkbox = vue.defineComponent({
      ...
      render() {
        var _this$$slots$default, _this$$slots;
        return vue.createVNode(vue.resolveComponent("ACheckboxGroup"), vue.mergeProps(this.$attrs, {
          "modelValue": this.value,
          "onUpdate:modelValue": this.onInput,
          "ref": "el"
        }), {
          default: () => [this.options().map((opt, index) => {
            return vue.createVNode(vue.resolveComponent("ACheckbox"), vue.mergeProps(opt, {
              "key": '' + index + '-' + opt.value
            }), {
              default: () => [opt.label || opt.value || '']
            });
    ...
    var Frame = vue.defineComponent({
      ...
      methods: {
        ...
        makeInput() {
          ...
          const slots = {
            append: () => vue.createVNode(Type, {
              "class": "_fc-frame-icon",
              "onClick": this.showModal
            }, null)
          };
          if (this.fileList.length && !this.disabled) {
            slots.suffix = () => vue.createVNode(script$3, {
              "class": "_fc-frame-icon",
              "onClick": () => {
                this.fileList = [];
                this.input();
        ...
        makeFiles() {
          return this.makeGroup(this.fileList.map((src, index) => {
            return this.makeItem(index, [vue.createVNode(script$1, {
      ...
      render() {
        ...
        }), vue.createVNode(vue.resolveComponent("aModal"), vue.mergeProps({
          width,
          title,
          ...this.modal
        }, {
    ...
    var components = [Upload, Sub, Group, Radio, Checkbox, Frame, script];
    ...
    var datePicker = {
      ...
      maker: function () {
        return ['date', 'month', 'week', 'year', 'quarter'].reduce((initial, mode) => {
          initial[mode] = creatorFactory(name$3, {
            mode
          });
          return initial;
        }, {
          dateRange: creatorFactory(name$3, {
            range: true
          }),
          datetimeRange: creatorFactory(name$3, m => m.props({
            range: true,
            showTime: true
          }))
        });
    ...
    var input = {
      ...
      render(children, ctx) {
        ...
        return ctx.$render.vNode.make('A' + alias$1[type], ctx.prop, children);
      }
    };
    ...
    var parsers = [datePicker, rangePicker, hidden, input, tree, select, row];
    ...
    var css_248z = ".form-create{width:100%}.form-create .fc-none,.form-create.is-preview .arco-form-item-label-required-symbol,.form-create.is-preview .fc-clock{display:none!important}";
